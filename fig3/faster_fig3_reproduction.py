...
def synthesize_gradients(tau_v, tau_g, H,
                         _phi_train_np=None, _y_logits_train_np=None):
    ...
    The expensive piece is building the per-class synthesized gradient set;
    we vectorize this by collecting all synthesizer eigenvectors into a
    single matrix U (m x s_total) and computing Phi_hat = Phi^T @ U as
    one matmul per class instead of inside Python loops.
    """
    # Caches passed in to avoid repeated .numpy() conversions across configs
    if _phi_train_np is None:
        _phi_train_np = [phi_train[c].numpy() for c in range(10)]
    if _y_logits_train_np is None:
        _y_logits_train_np = y_logits_train.numpy()
    ...
    eigvals_g = eigvals_g[::-1]
    eigvecs_g = eigvecs_g[:, ::-1]
    eigvals_g = np.maximum(eigvals_g, 0)

    cumvar = np.cumsum(eigvals_g) / max(eigvals_g.sum(), 1e-12)
    ...
    # Steps 4-5: build the synthesizer eigenvector matrix U (m x s_total)
    # Each column is one lifted eigenvector. We then synthesize all class
    # representatives at once via matmul.
    n_local = sum(local_ranks)
    n_gap   = len(gap_directions)
    n_total = n_local + n_gap
    if n_total == 0:
        return None, None, 0

    U = np.zeros((N_TRAIN_TOTAL, n_total))
    col = 0
    for h, Ih in enumerate(clusters):
    ...
    # Step 6: QR redundancy removal on U
    # Note: when s_total > m, np.linalg.qr returns R of shape (m, s_total)
    # but diag(R) only has length min(m, s_total) = m. The remaining
    # s_total - m columns are linearly dependent by definition (they live
    # in R^m). Pad the keep mask with False for those extra columns.
    Q, R = np.linalg.qr(U)
    ...
    if keep.size < U.shape[1]:
        keep = np.concatenate([keep, np.zeros(U.shape[1] - keep.size, dtype=bool)])
    U_keep = U[:, keep]                                  # (m, s_actual)
    s_actual = int(keep.sum())
    if s_actual == 0:
        return None, None, 0

    # Synthesize: Phi_hat^c = (Phi^c)^T @ U_keep, y_hat^c = Y^c^T @ U_keep
    # (one (s_actual, K_PROJ) matrix per class)
    U_keep_t = torch.from_numpy(U_keep).to(torch.float32)
    Phi_hat_final = []
    y_hat_final   = []
    for c in range(10):
        # phi_train[c] is (m, K_PROJ). We want U_keep^T @ Phi_train[c] => (s, K_PROJ).
        Phi_hat = U_keep_t.T @ phi_train[c]              # (s_actual, K_PROJ)
        y_hat   = U_keep_t.T @ y_logits_train[:, c]      # (s_actual,)
        Phi_hat_final.append(Phi_hat)
        y_hat_final.append(y_hat)
    return Phi_hat_final, y_hat_final, s_actual
